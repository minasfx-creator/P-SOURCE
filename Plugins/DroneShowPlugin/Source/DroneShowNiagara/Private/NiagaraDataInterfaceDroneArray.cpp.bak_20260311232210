...
{
    if (GWorld) return UDroneShowDataBridge::Get(GWorld);
    return nullptr;
}

void UNiagaraDataInterfaceDroneArray::VM_GetCount(FVectorVMContext& Context)
{
...
void UNiagaraDataInterfaceDroneArray::VM_GetTransformAtIndex(FVectorVMContext& Context)
{
    ...
    {
        OutPos.SetAndAdvance(FVector::ZeroVector);
        OutRot.SetAndAdvance(FVector::ZeroVector);
        OutIntensity.SetAndAdvance(0.0f);
        return;
    }
    OutPos.SetAndAdvance(Bridge->Positions[Index]);
    OutRot.SetAndAdvance(Bridge->RotationsEuler[Index]);
    OutIntensity.SetAndAdvance(Bridge->Intensities.IsValidIndex(Index) ? Bridge->Intensities[Index] : 1.0f);
}
